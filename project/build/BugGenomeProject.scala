...
class BugGenomeProject(info: ProjectInfo) extends ParentProject(info) with IdeaProject {
   ...
   class ExceptionMatcherProject(info: ProjectInfo) extends DefaultProject(info) with IdeaProject with ScctProject {
   ...
   lazy val restinterface = project("restinterface", "REST Interface", new DefaultProject(_) with IdeaProject with ScctProject, exceptionmatcher)
   lazy val webgui = project("webgui", "Web GUI", new DefaultProject(_) with IdeaProject with ScctProject) //web doesn't have any deps because it should use the REST interface
}
